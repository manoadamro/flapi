...
class IntTest(unittest.TestCase):
    ...
    def test_fails_if_float(self):
        prop = flapi.schema.types.Int()
        self.assertRaises(flapi.schema.errors.SchemaValidationError, prop, 1.5)

    # NUMBER TESTS

    def test_min_only(self):
        prop = flapi.schema.types.Int(min_value=0)
    ...
    def test_max_only(self):
        prop = flapi.schema.types.Int(max_value=10)
        self.assertEqual(prop(5), 5)
    ...
    def test_min_and_max(self):
        prop = flapi.schema.types.Int(min_value=0, max_value=10)
        self.assertEqual(prop(5), 5)
    ...
    def test_nullable_allows_null(self):
        prop = flapi.schema.types.Int(nullable=True)
        self.assertIsNone(prop(None))

    def test_nullable_raises_error(self):
        prop = flapi.schema.types.Int(nullable=False)
        self.assertRaises(flapi.schema.errors.SchemaValidationError, prop, None)

    def test_default_is_none(self):
        prop = flapi.schema.types.Int(default=None)
        self.assertIsNone(prop(None))

    def test_default_value(self):
        prop = flapi.schema.types.Int(default=12)
    ...
    def test_default_callable(self):
        prop = flapi.schema.types.Int(default=lambda: 12)
        self.assertEqual(prop(None), 12)

    def test_wrong_type(self):
        prop = flapi.schema.types.Int(callback=None)
        self.assertRaises(flapi.schema.errors.SchemaValidationError, prop, "nope")

    def test_callback(self):
        prop = flapi.schema.types.Int(callback=lambda v: v * 2)
